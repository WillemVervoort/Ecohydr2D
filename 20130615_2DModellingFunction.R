...
Gwt.fun <- function(t,last_t_heads, RB = gwheads[NX],first=T, 
		...
		if (first == T) { # only for first timestep
	  		...
	  		define_gwt_hydroinput(last_t_heads, bottomvector,
				ksat = aqK, spec_y = soilpar.m$spec_y)  #K_s from cm/d to m/d
#  	
...
big.fun <- function(N, stype, vtype, aqK_in, Rain, ETp, stream, gwheads, Zmean,
	...
	for (t in 1:N) {
		if (t == 1) {
			...
			GW.out <- Gwt.fun(t, last_t_heads = gwheads, 
				...
				aqK = aqK_in, GWcumvector = rep(0.0,NX), 
		...
		#	print(s_init) 
			...
			# if needed run gwt.exe function
				...
				aqK = aqK_in, GWcumvector=GW_store$GWcum[t-1,], 
				slopevector = slopevector.m,
				DELT.m = DELT,
				NRBL.m = NRBL)  
				# reset DELT
...
# Run gwt_fun one last time
	GW.out <- Gwt.fun(t = N+1, last_t_heads = GW_store$heads[N,], first=F,
				...
				aqK = aqK_in, GWcumvector=GW_store$GWcum[N,], 
				slopevector = slopevector.m,
				DELT.m = DELT,
				NRBL.m = NRBL)
	# write away gw output 
