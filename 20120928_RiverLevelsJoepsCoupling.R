...
Coupling <- function(N,stype,vtype,Rain,ETp,stream) {
  ...
  # This is a key variable based on Vervoort and van der Zee (2012)
	...
	##### Adjusted by Willem 20120928
	# define gw heads and also Zmean (based on Sebastian's data
	# NOTE THAT 0 is SOIL level, all depths are negative
	gwheads <-  -10.75 # this is in m, but Zmean should be in cm and positive
	# This is not very elegant, should actually be in vegpar, works for now
	# Maybe Zmean should be higher than the reported "rest level" assuming trees closer to river might take up water better
	Zmean <- gwheads*-100 + 675
...
# Joep used this to set the depth of the initial water table
deep <- FALSE

# TO MODEL DEEP ROOTS WITH TREES YOU NEED TO CHANGE vtype to
# vtype = "TreesDR" otherwise "TreesNoDR"
Coupling(N=250,stype="L Med Clay", vtype="TreesDR",Rain=Rain, ETp=ETp,stream=Stream)

# assign to look at output
result <- Coupling(N=nrow(Rain),stype="L Med Clay", vtype="TreesDR",Rain=Rain,ETp=ETp, stream=Stream)
...
# look at some output
Outfile <- read.csv("20121009_250_L Med Clay_TreesDR_Output_table.csv")
par(mar=c(4,4,2,5)) # Leave space for z axis
plot(Outfile$days,Outfile$Rain, type="h",xlab = "Time (days)", ylab="Rain (cm/d),Stage (m)")
lines(Outfile$days,(Outfile$River+3),col="purple",lwd=2)
par(new = TRUE)
plot(Outfile$days,Outfile$Heads.1, type = "l", axes=FALSE, bty = "n", 
	xlab = "", ylab="",col="red")
axis(side=4, at = pretty(range(Outfile$Heads.1)))
...
lgd.txt <- c("Rain","River stage",paste("heads",cumsum(c(20,30,50,100,100,200,200,300,500,1000,5000)),"m"))
