...
# 
# The base case after several tests is Res = 1/Ksoil, Kaq = KloamySand
# spec yield is specy Loamy sand. Res RBC is 0.5 (very quick)
# Starting depths as in Pfautsch et al
# This includes the spec y aquifer
# Zmean <- rep(650,42) #gw_in*-100 # could also try Zmean <- 500
# fs_veg <- 0.5
# Soil L Med Clay
...
# only capillary flow
veggies <- c(rep("TreesNoDR",10), veg)
soils <- "L Med Clay"
sp <- Soil("Loamy Sand") #Soil("L Med Clay")
#result <- big.fun(N=2,stype=soils, aqK_in = sp$K_s/100, aq_specy_in  = sp$spec_y,
#		 vtype=veggies,Rain=Rain, 
...
xyplot(gwlevel~as.Date(Dates), groups=as.factor(loc),data=sub1_Out, 
	type="l",strip=my.strip3,xlab =list( "Days",font=2,cex=1.2), 
	ylab = list("Groundwater level",font=2,cex=1.2),
	lwd=2,lty=c(1,1,2,2), col=c(1,"Gray50"), ylim=c(-10,-4),
	scales=list(x = list(font=2,cex=1.2),y = list(font=2,cex=1.2)),as.table=T,
	key=list(text=list(lab=paste("from river",c(0, 45, 85, 162.5),"m"),cex=1.2),
        lines=list(lwd=2,lty=c(1,1,2,2),col=c(1,"Gray50")),x = .53, y = .65, corner = c(0, 0)))
...
qqplot(Tobs$Control_Transpiration,OutTt.red*10,
	xlab="Observed transpiration (sapflow, mm/day)",
	ylab="Predicted total transpiration mm/day",
cex.lab=1.2,cex.axis=1.2,font.axis=2,font.lab=2)
lines(Tobs$Control_Transpiration,Tobs$Control_Transpiration,lty=2,lwd=2)
