# Script that includes better groundwater model with right hand boundary
...
Coupling <- function(N,stype,vtype,Rain,ETp,stream,gw_in,Zmean,pumprate=0) {
  ...
  #first define some initial values of BigFooFunction. Original vectors become matrices (1D to 2D)
  n <- soilpar$n
  ...
  ## Define all matrices for storage
...
# ++++ CHECK ++++++
 ...
 #Run Toons model once to get the proper head output / the first Z input
  ...
  while(i<=(nrow(ss)-1)&STOP==FALSE) {
	...
	# Run a for loop over all the locations
     for(j in 1:(NX*NY)) {

# ----------- Modification because we now run different veg on each location -------------
#            WV 20130531
		vegpar <- Veg(vtype=vtype[j], soilpar=soilpar)
		DR <- vegpar$DR
...
# execute GWT model one last time. + create final heads
   define_gwt_timestepinput(ITIM=(N+1), DELT=DELT, 
		RECH=format(t(GWcumvector)/DELT,nsmall=5),
		hriver=format(riverheads[floor(i*deltat)+1],nsmall=5)) # ++++++++++ CHECK THIS +++++++++++++
   system("GWT.exe")     
...
Zmean <- rep(400,NX) #gw_in*-100 # could also try Zmean <- 500
